...
(function () {
    ...
    function renderCorreos(emails) {
        ...
        emails.forEach(email => {
            ...
            card.className = isOut ? 'correo-card correo-card--out' : 'correo-card correo-card--in';
            
            // Un array recién descargado o sin propiedad assumes false (nuevo)
            ...
            const attachmentsHtml = email.attachments && email.attachments.length > 0
                ...
                ${email.attachments.map(a => `<a href="${escapeHtml(a.url)}" target="_blank" class="btn btn--sm btn--ghost" style="margin-right:6px; margin-bottom:6px;">${fileIcon(a.type)} ${escapeHtml(a.filename || a.name || 'Adjunto')}</a>`).join('')}</div>`
                : '';

            const dotActive = isUnread ? 'background-color: var(--primary); box-shadow: 0 0 8px var(--primary);' : 'background-color: var(--text-muted); opacity: 0.3;';
            const pillHtml = isUnread ? `<span class="correo-card__new-pill" style="background:var(--primary); color:white; font-size:0.65rem; padding: 2px 6px; border-radius:4px; margin-right:6px; vertical-align:middle; font-weight:bold;">NUEVO</span>` : '';
            ...
            card.innerHTML = `
                <div class="correo-card__header">
                    <div style="display:flex; align-items:center; gap: 12px; flex:1; min-width:0;">
                        <span class="correo-card__dot" style="${dotActive}"></span>
                        <div style="min-width:0;">
                            <p class="correo-card__subject">${pillHtml}${escapeHtml(email.subject || '(Sin asunto)')}</p>
                ...
                <div class="correo-card__body" style="display:none;">
                    <p class="correo-card__text" style="white-space:pre-wrap;">${linkify(escapeHtml(email.body || '(Sin contenido)'))}</p>
                    ${attachmentsHtml}
                    ${actionsHtml}
                </div>
            `;
            ...
            toggleBtn.addEventListener('click', async () => {
                ...
                // Si se acaba de abrir por primera vez, marcar como leído
                if (!isOpen && isUnread && email.id) {
                    email.isRead = true; // Actualiza el estado local rápido
                    
                    // Modificar UI instántaneamente
                    const pill = card.querySelector('.correo-card__new-pill');
                    if (pill) pill.style.display = 'none';
                    const dot = card.querySelector('.correo-card__dot');
                    if (dot) {
                    ...
                    // Enviar confirmación al backend de manera silenciosa
                    try {
                        await apiPost('mark_email_read', { id: email.id });
                    } catch (e) {
                        console.error('Error al marcar correo como leído', e);
                    }
                }
            });

                    const btnReply = card.querySelector('.btn-email-reply');
                    if (btnReply) {
                        btnReply.addEventListener('click', (e) => {
                            ...
                            if (window.openEmailModal) window.openEmailModal('reply', email.id, subject, body);
                        });
                    }

                    const btnForward = card.querySelector('.btn-email-forward');
                    if (btnForward) {
                        btnForward.addEventListener('click', (e) => {
                            e.stopPropagation();
                            const subject = email.subject.toLowerCase().startsWith('fwd:') ? email.subject : `Fwd: ${email.subject}`;
                            const body = `\n\n\n--- Mensaje reenviado ---\nFecha: ${email.date}\nDe: Irene\n\n${email.body || email.text || ''}`;
                            if (window.openEmailModal) window.openEmailModal('forward', email.id, subject, body);
                        });
                    }
    ...
    if ($btnSyncCorreos) {
        $btnSyncCorreos.addEventListener('click', async () => {
            ...
            $btnSyncCorreos.innerHTML = '<div class="premium-spinner"></div> Sincronizando...';
            correosLoaded = false;
            try {
    ...
    window.openEmailModal = function(type = 'new', replyId = '', initialSubject = '', initialBody = '', initialTo = '') {
        const modal = document.getElementById('email-modal-overlay');
        document.getElementById('email-compose-type').value = type;
        document.getElementById('email-compose-reply-to-id').value = replyId;
        document.getElementById('email-compose-subject').value = initialSubject;
        document.getElementById('email-compose-body').value = initialBody;
        if (initialTo) {
            document.getElementById('email-compose-to').value = initialTo;
        }
        document.getElementById('email-compose-attachments').value = '';
        if (modal) modal.style.display = 'flex';
    };

    const $btnNewCorreo = document.getElementById('btn-new-correo');
    if ($btnNewCorreo) $btnNewCorreo.addEventListener('click', () => window.openEmailModal('new'));
    ...
    const $emailBtnCancel = document.getElementById('email-btn-cancel');
    if ($emailModalClose) $emailModalClose.addEventListener('click', closeEmailModal);
    if ($emailBtnCancel) $emailBtnCancel.addEventListener('click', closeEmailModal);

    const $emailComposeForm = document.getElementById('email-compose-form');
    if ($emailComposeForm) {
        $emailComposeForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            const btnSend = document.getElementById('email-btn-send');
            const originalContent = btnSend.innerHTML;
            btnSend.disabled = true;
            btnSend.innerHTML = '<div class="premium-spinner"></div> Enviando...';

            try {
                const formData = new FormData();
                formData.append('type', document.getElementById('email-compose-type').value);
                formData.append('to', document.getElementById('email-compose-to').value);
                formData.append('reply_to_id', document.getElementById('email-compose-reply-to-id').value);
                formData.append('subject', document.getElementById('email-compose-subject').value);
                formData.append('body', document.getElementById('email-compose-body').value);
                
                const files = document.getElementById('email-compose-attachments').files;
                if (files && files.length > 0) {
